          ...
          <div className="flex items-start gap-6 flex-wrap">
            ...
            <label className="relative w-20 h-20 rounded-2xl overflow-hidden cursor-pointer group shrink-0" style={{ boxShadow: '4px 4px 8px rgba(163, 177, 198, 0.6), -4px -4px 8px rgba(255, 255, 255, 0.8)' }}>
              {(editing ? form.avatar : profile.avatar) ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[#1a202c] text-2xl font-bold" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)' }}>
            ...
            <div className="flex-1">
              {editing ? (
                <div className="space-y-3">
                  <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Name" className="w-full px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none placeholder-[#a0aec0]" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  <input type="text" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="Username" className="w-full px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none placeholder-[#a0aec0]" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="w-full px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none placeholder-[#a0aec0]" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} placeholder="Bio" rows={2} className="w-full px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none placeholder-[#a0aec0] resize-none" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  <input type="text" value={form.learningGoal} onChange={(e) => setForm({ ...form, learningGoal: e.target.value })} placeholder="Learning Goal" className="w-full px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none placeholder-[#a0aec0]" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  <div>
                    <label className="text-xs text-[#718096] mb-1 block font-medium">Daily Goal (hours)</label>
                    <input type="number" min={1} max={24} value={form.dailyGoal} onChange={(e) => setForm({ ...form, dailyGoal: parseInt(e.target.value) || 1 })} className="w-24 px-3 py-2 rounded-xl text-[#1a202c] text-sm focus:outline-none" style={{ background: '#e6ebf2', boxShadow: 'inset 3px 3px 6px rgba(163, 177, 198, 0.5), inset -3px -3px 6px rgba(255, 255, 255, 0.9)', border: '1px solid rgba(255,255,255,0.6)' }} />
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setEditing(false); setForm(profile); }} className="px-4 py-2 rounded-xl text-[#718096] hover:text-[#1a202c] text-sm cursor-pointer flex items-center gap-1" style={{ background: '#e6ebf2', boxShadow: '3px 3px 6px rgba(163, 177, 198, 0.5), -3px -3px 6px rgba(255, 255, 255, 0.8)', border: '1px solid rgba(255,255,255,0.7)' }}><X className="w-4 h-4" /> Cancel</button>
                  </div>
                </div>
              ) : (
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h2 className="text-xl font-bold text-[#1a202c]">{profile.name}</h2>
                    <button onClick={() => { setEditing(true); setForm(profile); }} className="text-[#718096] hover:text-[#1a202c] cursor-pointer"><Edit3 className="w-4 h-4" /></button>
                  </div>
                  {profile.username && <p className="text-sm text-[#718096]">@{profile.username}</p>}
                  ...
                  <p className="text-[11px] text-[#718096] mt-2">Joined {formatDate(profile.createdAt, 'MMMM yyyy')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            <div key={s.label} className="profile-card bg-[#e6ebf2] border border-[rgba(255,255,255,0.7)] rounded-2xl p-4 text-center" style={{ boxShadow: '6px 6px 14px rgba(163, 177, 198, 0.6), -6px -6px 14px rgba(255, 255, 255, 0.85)' }}>
              ...
              <AnimatedCounter value={s.value} suffix={s.suffix} className="text-lg font-bold text-[#1a202c] block" />
              <p className="text-[10px] text-[#718096] mt-1">{s.label}</p>
            </div>
          ))}
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {stats.badges.map((badge) => (
                <div key={badge.name} className="flex flex-col items-center gap-2 p-4 rounded-xl transition-all" style={{ background: '#e6ebf2', boxShadow: '4px 4px 8px rgba(163, 177, 198, 0.5), -4px -4px 8px rgba(255, 255, 255, 0.8)', border: '1px solid rgba(255,255,255,0.7)' }}>
                  ...
                  <span className="text-xs font-medium text-[#1a202c] text-center">{badge.name}</span>
                  <span className="text-[10px] text-[#718096] text-center">{badge.desc}</span>
                </div>
              ))}
            </div>
          )}
